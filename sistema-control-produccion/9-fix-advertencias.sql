-- ============================================================
-- FIX: "literal de matriz malformada" al guardar
--
-- adv := adv || 'texto' es ambiguo en Postgres: puede interpretar el texto
-- como si fuera otra lista y fallar al parsearlo. array_append no tiene esa
-- ambiguedad. Correr en el SQL Editor.
-- ============================================================
...
begin
  ...
  if v_prod is not null then
    adv := array_append(adv, 'Ya existia un parte para esa rollera, fecha y turno: se agregan las bobinas nuevas');
  else
  ...
  for b in select * from jsonb_array_elements(coalesce(datos->'bobinas','[]'::jsonb)) loop
    ...
    if exists (select 1 from bobinas
      ...
      adv := array_append(adv, 'Bobina ya cargada (mismo numero y peso), se omitio: ' || (b->>'n_bobina'));
      continue;
    end if;
