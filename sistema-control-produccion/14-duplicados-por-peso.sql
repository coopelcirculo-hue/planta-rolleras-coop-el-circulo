-- ============================================================
-- 14 · RECONOCER UNA HOJA REPETIDA POR LOS PESOS
--
-- El control del SQL 13 reconoce una hoja ya cargada comparando los NUMEROS de
-- bobina. Pero cuando la IA lee mal los numeros ("525" por "7-25") o la fecha
-- (la deja en la de hoy), la hoja repetida entraba como nueva.
--
-- Ahora tambien se la reconoce por los PESOS: si casi todos los pesos de la foto
-- (80% o mas, minimo 3) estan exactos en una hoja de la misma rollera cargada en
-- el ultimo mes y medio, y la cantidad de bobinas es parecida, es la misma hoja.
-- Nueve pesos exactos iguales no se repiten por casualidad.
--
-- Correr en el SQL Editor de Supabase. Se puede repetir sin riesgo.
-- ============================================================
...
begin
  ...
  -- c) Otra hoja de la misma rollera, dia y turno con OTRO operario: se guarda,
  --    pero marcada, porque casi siempre es la fecha o el turno mal leidos.
